...
def main():
    ...
    for l in range(n_params):
        ...
        for i in range(11):
            print(l,i)
            for j in range(12):
                Z[i,j] = black_box_function(x_array[i],y_array[j],l)

        np.savetxt("data/X_"+str(l)+".csv", X, delimiter=",",fmt ='% s')
        np.savetxt("data/Y_"+str(l)+".csv", Y, delimiter=",",fmt ='% s')
        np.savetxt("data/Z_"+str(l)+".csv", Z, delimiter=",",fmt ='% s')
