...
def main():
    ...
    ax.plot_surface(X, Y, Z.T, zorder=3, color='orange')

    ax.scatter(X[0,x], Y[y,0], z, color='red', s=100, zorder=2)

    ax.text(X[0,x], Y[y,0], z+100,  'skirt_r = %s, theta = %s2' % (str(X[0,x]), str(round(Y[y,0], 4))) , size=20, zorder=1,
    color='k')
    ...
    rotanimate(ax, angles,'grid_search.gif',delay=20)
